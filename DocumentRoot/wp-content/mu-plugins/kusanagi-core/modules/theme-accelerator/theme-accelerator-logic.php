...
if (!defined('CLEAN_ACTION')) {
    define('CLEAN_ACTION', 'kusanagi-cache-clear');
}
if (!defined('CLEAN_NAME')) {
    define('CLEAN_NAME', 'kusanagi-cache-clear');
}
...
class KUSANAGI_Theme_Accelerator_Logic {
    ...
    public static function theme_accelerator_start() {
        ...
        if (self::is_block_templates()) {
            KUSANAGI_Theme_Accelerator_Logic::fse_templates_cache();
        }

        $has_cache = self::$ApcuInstance->get('need_cache' . self::$SiteUri);
        add_action('init', function () {
            ob_start(array(__CLASS__, 'ob_init'));
        }, self::HOOKSEQ_LAST);

        add_action('plugins_loaded', array( __CLASS__, 'wp_ajax_clear_cache'), -1);
        add_action('shutdown', function () use (&$has_cache) {
            global $wp_filter;
            if (empty($has_cache)) {
                $need_cache = array();
                foreach ($wp_filter as $key => $value) {
                    if (strpos($key, 'footer') !== false && strpos($key, 'admin') === false) {
                        $need_cache[] = $key;
                    }
                    if (strpos($key, 'header') !== false && strpos($key, 'admin') === false) {
                        $need_cache[] = $key;
                    }
        ...
        add_action('wp_head', array(__CLASS__, 'theme_accelerator_first'), self::HOOKSEQ3);
        add_action('wp_head', array(__CLASS__, 'theme_accelerator_end'),self::HOOKSEQ_LAST );
        add_action('generate_before_footer', array(__CLASS__, 'theme_accelerator_first'), self::HOOKSEQ3);
        add_action('generate_before_footer', array(__CLASS__, 'theme_accelerator_end'),self::HOOKSEQ_LAST );
        ...
        if (!empty($has_cache)) {
            foreach ($has_cache as $key => $value) {
                ...
                add_action($value, array(__CLASS__, 'theme_accelerator_first'), self::HOOKSEQ3);
                add_action($value, array(__CLASS__, 'theme_accelerator_end'),self::HOOKSEQ_LAST );
            }
        }

        add_action('wp_head', array(__CLASS__, 'head_not_cache'), self::HOOKSEQ1);
    }
    ...
    public static function theme_accelerator_first() {
        ...
        if (!empty($cacheData) && 'generate_footer' !== $current_action) {
            ...
            if (strpos($all_ob[sizeof($all_ob) - 1], 'ob_init') !== false) {
                ob_start(function ($data) use (&$current_action) {
                    self::$ApcuInstance->set($current_action . self::$RequestUri, $data);

                    return $data;
                });
            } else {
                remove_filter($current_action, array(__CLASS__, 'theme_accelerator_end'),self::HOOKSEQ_LAST );
            }
        }
    }
    ...
    public static function is_valid_access() {
        return isset($_GET[CLEAN_NAME]) && wp_verify_nonce($_GET[CLEAN_NAME], CLEAN_ACTION);
    }
    ...
    public static function head_not_cache() {
        ...
        foreach ($wp_filter['wp_head']->callbacks as $key => $val) {
            foreach ($val as $key2 => $value2) {
                if (strpos($key2, 'styles') !== false || strpos($key2, 'scripts') !== false || strpos($key2, 'admin') !== false) {
        ...
        remove_action('wp_head', array(__CLASS__, 'head_not_cache'), self::HOOKSEQ1);
    }
    ...
    public static function fse_templates_cache() {
        add_filter('pre_get_block_templates', function ($a, $b, $c) {
            $cache_key = '';
            $result = array();
            if (!empty($b)) {
                array_walk_recursive($b, function ($v, $key) use (&$result) {
                    $result[] = $key . $v;
                });
        ...
        add_filter('get_block_templates', function ($a, $b, $c) {
            $cache_key = '';
            $result = array();
            if (!empty($b)) {
                array_walk_recursive($b, function ($v, $key) use (&$result) {
        ...
        add_filter('pre_get_block_template', function ($a, $b, $c) {
            $cache_key = '';
            $result = array();
            if (!empty($b)) {
                array_walk_recursive($b, function ($v, $key) use (&$result) {
        ...
        add_filter('get_block_template', function ($a, $b, $c) {
            $cache_key = '';
            $result = array();
            if (!empty($b)) {
                array_walk_recursive($b, function ($v, $key) use (&$result) {
        ...
        //render部分
        add_filter('pre_render_block', function ($a, $b, $father) {
            global $post;
            if (empty($father)) {
                return null;
            }
            if (!in_array($b['blockName'], self::$render_cache_key) && !in_array($b['blockName'], self::$render_cache_key_need_post_id)) {
                return null;
            }
            if (in_array($b['blockName'], self::$render_cache_key_need_post_id)) {
            ...
            $result = array();
            if (!empty($attributes)) {
                array_walk_recursive($attributes, function ($v, $key) use (&$result) {
                    $result[] = $key . $v;
                });
                $cache_key = http_build_query($result, '', '____');
            }
            ...
            if (!empty($context['postId'])) {
                array_walk_recursive($context, function ($v, $key) use (&$result) {
        ...
        add_filter('render_block', function ($a, $b) {
            global $post;
            if (!in_array($b['blockName'], self::$render_cache_key) && !in_array($b['blockName'], self::$render_cache_key_need_post_id)) {
                return $a;
            }
