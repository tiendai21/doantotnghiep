...
<?php endforeach; ?>
			...
			<h3><?php _e( 'Theme Accelerator', 'wp-kusanagi' ); ?></h3>
			<table class="form-table">
			<tr>
				<th><?php _e( 'Enable Theme Accelerator', 'wp-kusanagi' ); ?></th>
				<td>
					<input type="hidden" name="opt-speed-up[enable]" value="0">
					<label for="opt-speed-up-enable">
						<input type="checkbox" name="opt-speed-up[enable]" id="opt-speed-up-enable" value="1"<?php echo $s['opt-speed-up']['enable'] ? ' checked="checked"' : ''; ?>>
						<?php _e( 'Enable', 'wp-kusanagi' ); ?>
					</label>
					<p class="description">
					<?php _e( 'Theme Accelerator caches functions that need to be accelerated. At the same time, it will speed up themes that supports FSE.', 'wp-kusanagi' ); ?>
					</p>
				</td>
			</tr>
			</table>