...
if (array_key_exists($keyIndex - 1, $kakuwa_posts) || array_key_exists($keyIndex + 1, $kakuwa_posts)) {
    ...
    echo '<div class="program-stepper">';
    echo '<div class="wrap">';

    if (array_key_exists($keyIndex + 1, $kakuwa_posts)) {
        echo '<div class="button prev"><a href="' . $kakuwa_posts[$keyIndex + 1]['permalink'] . '">前へ</a></div>';
    }
    if (array_key_exists($keyIndex - 1, $kakuwa_posts)) {
        echo '<div class="button next"><a href="' . $kakuwa_posts[$keyIndex - 1]['permalink'] . '">次へ</a></div>';
    }
    echo '</div>';
    echo '</div>';
}
